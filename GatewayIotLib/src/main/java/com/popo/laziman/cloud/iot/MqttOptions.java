...
public class MqttOptions {
	...
	public static MqttOptions fromFlags(String[] args) {
		...
		options.addOption(Option.builder().type(String.class).longOpt("gateway_id").hasArg().desc("The identifier for the Gateway.").build());
		...
		// Optional arguments.
		options.addOption(Option.builder().type(String.class).longOpt("command").hasArg().desc("Command to run:" + "\n\tlisten-for-config-messages" + "\n\tsend-data-from-bound-device").build());
		options.addOption(Option.builder().type(String.class).longOpt("telemetry_data").hasArg().desc("The telemetry data (string or JSON) to send on behalf of the delegated device.").build());
